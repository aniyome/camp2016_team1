            ...
            for(int i = 0; i < splitSprites.Count; i++)
                if(percentage < 1 && (i + 1) * iconPercentage > percentage)//if this heart should fade.
                {
                    if(i * iconPercentage <= percentage)
                        splitSprites[i].SetValue((percentage % iconPercentage) / iconPercentage + 0.0001f);//show the heart what percentage it should display. We add a small amount on to help with floating point error
                    else
                        splitSprites[i].SetValue(0);//this is sprites past the current value and should be empty
                }
                else
                    splitSprites[i].SetValue(1);
